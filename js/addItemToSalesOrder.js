...
function addItemToSalesOrderList(event){
    ...
    list_item.setAttribute("onclick", "deleteButton(this)");
    // Adds product name to list item.
    list_item.innerHTML += `<div class="order_item_padding order_item_ellipsis order_item_layout">${event.textContent}</div>`
    list_item.innerHTML += `<div class="price order_item_padding order_item_layout">${parseFloat(event.value).toFixed(2)}</div>`;
    list_item.innerHTML += `<div class="order_item_delete order_item_padding">x</div>`;
